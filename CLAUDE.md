...
- **src/xdp.rs** + **build.rs** — `Blocker` trait. On Linux + `xdp` feature, `build.rs` compiles `src/bpf/xdp.c` (BTF maps, clang) to `$OUT_DIR/xdp.o`; `XdpBlocker` loads it via `aya` and attaches to the interface; elsewhere a no-op stub. `block_ip` writes IP→1 into the `blocklist` map using the Go key encoding. XDP init failure is non-fatal (logged, proxy continues without L4). The `xdp_stats` map carries a per-reason drop breakdown (blocklist/udp/tcp_malformed/http_invalid/tls_invalid/icmp/bad_flags/fragment/amplify/syn_flood) and a `fingerprints` LRU map records FNV-1a hashes + the first 16 payload bytes of dropped packets (`top_fingerprints`/`clear_fingerprints`). The `BpfStats`/`BpfFingerprint` reprs must stay byte-identical to the C structs. L4 SYN-flood defense is per-source SYN rate limiting (`check_syn_rate`, 100 SYN/s/IP) plus first-data-packet HTTP/TLS validation before a flow enters `allowed_flows` — no handshake interception (an experimental RST-cookie mode was removed because it broke legitimate browsers).
