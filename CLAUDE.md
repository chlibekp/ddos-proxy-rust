...
- **src/netmatch.rs** — `IpCidr` parse/match (IPv4+IPv6) backing the `PROXY_TRUSTED_IPS` allowlist and `PROXY_DENY_IPS` denylist.
- **src/waf/** — the core. `Manager::handle` reproduces the Go middleware branch-for-branch, with pre-filters in front: global in-flight cap → IP denylist → trusted-IP bypass (config + runtime lists; trusted skips maintenance too) → maintenance mode → method allowlist → URI-length cap → body-size cap → host allowlist → blocked paths / block regex (normalized path) → websocket bypass → require-UA / blocked UA / whitelist UA → basic-auth gate → honeypot block → per-IP concurrency cap → blocked fast-path → exempt paths → verified fast-path → mitigation eval (per-IP + per-path rate limits; adaptive PoW difficulty under mitigation) → challenge/block → proxy (404-scanner tracking on the response). `ClientState` (`client.rs`) keeps lock-free atomic fast-path fields (`blocked_flag`, `verified_flag`, `verified_until`, `last_seen`) in sync with a `Mutex<Inner>`. Per-client state lives in a `DashMap` keyed `IP|Host`. PoW = SHA-256 over `salt+nonce` with `pow_difficulty` leading zero hex chars; verify at `POST /challenge/verify` rejects sub-2s solutions. Turnstile via reqwest. Challenge HTML rendered with **minijinja** from `challenge.html` (Go template directives converted to Jinja).
...
- **src/proxy.rs** — reverse proxy over a pooled `hyper_util` client with `hyper-rustls` (http+https backends). Header rewrite (preserve `Host`, append `X-Forwarded-For`, set `X-Real-IP`/`X-Forwarded-Host/Proto`, strip hop-by-hop, drop `Accept-Encoding` for HTML), `Server`→`ddos-proxy`, `Via`, `X-Ddos-Proxy-Cache` (HIT/MISS/DYNAMIC/STALE), redirect `Location` rewrite, and `<head>/<body>` JS injection (gzip-aware) using the byte-identical `JS_SNIPPET`. Resilience layer: backend timeout (504), bounded GET/HEAD retries, circuit breaker (fail-fast 503), serve-stale-on-error from the disk cache. Optional response gzip, CORS, custom add/remove headers, `X-Request-Id`. WebSocket upgrades are tunneled via a manual HTTP/1 handshake + `copy_bidirectional`.
...
- **src/tls.rs** — on-demand ACME via **instant-acme** + **rcgen**. `OnDemandResolver` (rustls `ResolvesServerCert`) serves cached certs and triggers background issuance per SNI for any hostname (global issuance rate limit + per-host inflight/backoff dedup, disk cache under `certs/`, 24h renew-before, retry backoff). HTTP-01 served by the redirect server on `http_port`. Supports staging, custom directory, and EAB.
- **src/xdp.rs** + **build.rs** — `Blocker` trait. On Linux + `xdp` feature, `build.rs` compiles `src/bpf/xdp.c` (BTF maps, clang) to `$OUT_DIR/xdp.o`; `XdpBlocker` loads it via `aya` and attaches to the interface; elsewhere a no-op stub. `block_ip` writes IP→1 into the `blocklist` map using the Go key encoding. XDP init failure is non-fatal (logged, proxy continues without L4). The `xdp_stats` map carries a per-reason drop breakdown (blocklist/udp/tcp_malformed/http_invalid/tls_invalid) plus `syn_challenged`/`syn_validated` counters, and a `fingerprints` LRU map records FNV-1a hashes + the first 16 payload bytes of dropped packets (`top_fingerprints`/`clear_fingerprints`). The `BpfStats`/`BpfFingerprint`/`BpfConfig` reprs must stay byte-identical to the C structs. **SYN-cookie (RST-cookie) auth** (`PROXY_XDP_SYN_AUTH`, off by default): when the aggregate SYN rate crosses `PROXY_XDP_SYN_AUTH_PPS`, the program answers new SYNs with a bogus SYN-ACK via `XDP_TX` (in-place rewrite + recomputed IP/TCP checksums) carrying a tuple+secret cookie in the ACK field; a genuine client's RST (seq == cookie) whitelists its IP in `syn_authed` so its SYN retransmit passes. `init_xdp(iface, SynAuthConfig)` writes the enable flag, pps threshold, and a random cookie secret into the `xdp_cfg` map at load.
